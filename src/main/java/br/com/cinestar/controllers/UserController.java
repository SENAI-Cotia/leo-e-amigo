...
public class UserController {
    ...
    @GetMapping
    public String getUsers(Model model, @RequestParam(value = "error", required = false) String error, @RequestParam(value = "logout", required = false) String logout){
    ...
    public String salvarEdicaoUsuario(@ModelAttribute User userR, @RequestParam(value = "novaSenha", required = false) String novaSenha, RedirectAttributes redirectAttributes) {
        User userExistente = userRepository.findById(userR.getId()).orElseThrow(() -> new RuntimeException("Usuário não encontrado"));

        //O emailExistente não deixa editar se tentar colocar email repetido
        Optional<User> emailExistente = userRepository.findByEmail(userR.getEmail());
        if (emailExistente.isPresent() && !emailExistente.get().getId().equals(userR.getId())) {
            redirectAttributes.addFlashAttribute("mensagemErro", "E-mail já está em uso.");

            return "redirect:/editarUsuario/" + userR.getId();
        }
        //Aqui a mesma coisa, o usernameExistente não deixa editar se tentar colocar username repetido
        Optional<User> usernameExistente = userRepository.findByUsername(userR.getUsername());
        if (usernameExistente.isPresent() && !usernameExistente.get().getId().equals(userR.getId())) {
            redirectAttributes.addFlashAttribute("mensagemErro", "Username já está em uso.");
    ...
    public String excluirUsuario(@PathVariable long id) {
        reviewRepository.deleteAll(reviewRepository.findByUser(userRepository.findById(id).orElseThrow(() -> new RuntimeException("Usuário não encontrado"))));
